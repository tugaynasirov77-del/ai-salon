    ...
    <section id="savings" className="relative py-24">
      <div className="mx-auto max-w-5xl px-4">
        ...
        <div className="mt-12 grid grid-cols-1 gap-6 lg:grid-cols-[1fr_1fr]">
          ...
          {/* Results */}
          <div className="relative overflow-hidden rounded-2xl border border-[#8A8E96]/20 bg-gradient-to-br from-amber-950/40 via-amber-950/30 to-amber-900/30 p-6 backdrop-blur-sm sm:p-8">
            <div className="absolute -top-20 right-0 h-[200px] w-[200px] rounded-full bg-[#5A5E66]/30 blur-[80px]" />
  ...
  return (
    <div className="mt-6">
      ...
      <input
        ...
        className="mt-2.5 h-1.5 w-full cursor-pointer appearance-none rounded-full bg-white/10 accent-[#8A8E96]"
        style={{
          background: `linear-gradient(to right, #8A8E96 0%, #C0C4CB ${((value - min) / (max - min)) * 100}%, rgba(255,255,255,0.1) ${((value - min) / (max - min)) * 100}%)`,
        }}
      />
    </div>
  );
