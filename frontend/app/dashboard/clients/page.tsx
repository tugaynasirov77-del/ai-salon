        ...
        {clientsQ.isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="border-b border-slate-200 bg-slate-50 text-left text-xs uppercase tracking-wider text-slate-500 dark:border-slate-800 dark:bg-[#1A1612]/50">
                <tr>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {filtered.map((c) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-5 py-3 font-medium text-[#1A1612] dark:text-slate-100">
                        {c.name || '—'}
                      </td>
...
function ClientDrawer({
  ...
  return (
    <>
      ...
      <aside className="fixed right-0 top-0 z-50 flex h-full w-full max-w-md flex-col border-l border-slate-200 bg-white shadow-xl dark:border-slate-800 dark:bg-[#1A1612]">
        <header className="flex items-start justify-between border-b border-slate-100 px-5 py-4 dark:border-slate-800">
          <div>
            <div className="text-lg font-semibold text-[#1A1612] dark:text-slate-100">
                ...
                {appointments
                  ...
                  .map((a) => (
                    <li key={a.id} className="rounded-lg border border-slate-200 p-3 text-sm dark:border-slate-800">
                      <div className="font-medium text-[#1A1612] dark:text-slate-100">{a.service}</div>
            ...
            {messages.length === 0 ? (
              ...
              <ul className="space-y-2">
                {messages.map((m) => (
                  ...
                  >
                    <div className="text-[#1A1612] dark:text-slate-100">{m.text}</div>
