        ...
        {services.isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="border-b border-slate-200 bg-slate-50 text-left text-xs font-medium uppercase tracking-wider text-slate-500 dark:border-slate-800 dark:bg-[#1F232A]/50">
                <tr>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {list.map((s) => {
                  ...
                  return (
                    <tr key={s.id} className={cn('hover:bg-slate-50 dark:hover:bg-[#1F232A]/40', !s.isActive && 'opacity-60')}>
                      <td className="px-4 py-3 font-medium text-[#1F232A] dark:text-slate-100">{s.name}</td>
