...
function ChannelHeader({ icon: Icon, title, status }: { icon: React.ComponentType<{ className?: string }>; title: string; status?: React.ReactNode }) {
  return (
    <div className="mb-3 flex items-center gap-2">
      ...
      <h3 className="text-sm font-semibold text-[#1E2329] dark:text-slate-100">{title}</h3>
      {status}
    </div>
  );
}
  ...
  return (
    <Card>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <div>
          <Label>Превью</Label>
          <div className="relative h-64 overflow-hidden rounded-lg border border-slate-200 bg-gradient-to-br from-slate-50 to-slate-100 dark:border-slate-800 dark:from-[#1E2329] dark:to-[#14181F]">
      ...
      <div className="mt-6">
        ...
        <div className="flex flex-wrap items-stretch gap-2">
          <pre className="flex-1 min-w-0 overflow-x-auto rounded-lg bg-[#1E2329] px-3 py-2 text-xs text-slate-100">
            <code>{snippet}</code>
          </pre>
...
function AccountTab({ email, notify }: { email?: string; notify: (k: 'ok' | 'err', m: string) => void }) {
  ...
  return (
    <Card>
      ...
      <div className="mb-6">
        <Label>Email</Label>
        <Input value={email || ''} readOnly disabled className="cursor-not-allowed bg-slate-50 dark:bg-[#1E2329]/50" />
